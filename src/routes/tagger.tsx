...
export default function Tagger() {
    ...
    const removeTagFromImage = createMutation<any, any, { imageIdx: number; tag: string }>(() => ({
    ...
    const addTagToImage = createMutation<any, any, { imageIdx: number; tag: string }>(() => ({
    ...
    createEffect(async () => {
        if (files.data && files.data) {
            ...
            } else if (currentImageIdx() + FETCH_WHEN_LESSTHAN >= files.data?.length) {
                                        ...
                                        <div class={"mt-6"}>
                                            <Switch
                                                fallback={
                                                    ...
                                                    <img height={"500px"} src={CDN_URL + file.Key} class={"h-[70vh]"} />
                                                }>
                    ...
                    <div class={"flex gap-2"}>
                        <For each={tags.data}>
                            {(tag) => (
                                <Button
                                    class={cn({
                                        "bg-primary text-primary-foreground": selectedTags().includes(tag.name),
                                        "bg-primary-foreground text-primary": !selectedTags().includes(tag.name),
                                    })}
                                    onClick={(e) => {
                                        e.preventDefault();
                                        if (selectedTags().includes(tag.name)) {
                                            removeTagFromImage.mutate({ imageIdx: currentImageIdx(), tag: tag.name });
                                        } else {
                                            addTagToImage.mutate({ imageIdx: currentImageIdx(), tag: tag.name });
                                        }
                                        setLastViewed.mutate({ startAfter: files.data![currentImageIdx()].Key! });
                                    }}>
                                    {tag.name}
                                </Button>
                            )}
